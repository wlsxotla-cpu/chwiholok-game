...
const VERSION := "v0.51.12 · 2026-09-21"
...
const PET_DEFS := {
	...
	"freeze_charm": {"name": "빙결부", "sprite": "res://assets/sprites/pet_spirit_freeze.png", "offset": Vector2(-34, -92), "cost": 2000, "consumable": true, "desc": "내공 2000 소모해 1개 구매하는 소모품 (최대 99개 보유). 보유한 개수만큼 판에 데려갈 수 있고, 한 판당 1개씩 소모됨. 그 판 안에서는 좌측 하단 스킬 버튼으로 화면 내 모든 적을 2초간 완전히 얼려버릴 수 있음 (단, 빙궁 보스에게는 통하지 않음) (재사용 대기시간 30초)"},
}
...
func can_buy_pet(id: String) -> bool:
	if is_consumable_pet(id):
		return pet_stock_count(id) < MAX_PET_STOCK and total_coins >= pet_cost(id)
