...
const BASE_WEAPON_GUIDE := [
	...
	["fireball", "화염구 장판", "적 위치에 불바다 장판을 소환해 지속 데미지 + 밟은 적 이동속도 감소."],
	...
	["halberd", "패왕붕권", "주먹 모양의 기를 정면으로 발사. 레벨이 오를수록 세로로 나란히 발사되는 기의 개수가 늘어남. 진악 전용 시작 무기."],
	["curse", "귀곡저주", "가장 위협적인 적(보스 우선)을 저주로 낙인찍어 지속적으로 갉아먹음. 보스에게는 추가로 최대체력 비례 피해. 사마획 전용 시작 무기."],
]

const FUSION_GUIDE := [
	["heaven_blade", "천지개벽검", "회전베기 + 호신강기", "더 크고 강한 범위 베기 + 강력한 넉백. 보유 중엔 적의 원거리 공격을 1.2초에 한 번 검으로 막아냄"],
	["piercing_calamity", "멸겁관천검", "관통시 + 일자검기", "사거리 훨씬 긴 관통 검기, 적중한 적은 3초간 받는 피해 40% 증가"],
	["whirl_storm", "선풍만리표", "표창난사 + 회류표", "제자리에서 회전하며 회오리를 일으킨 뒤 사방으로 튕겨나가는 칼날"],
	["thunder_formation", "뇌검진", "어검비행 + 뇌전장", "회전검 + 번개 낙뢰를 동시 운용하는 복합 무기"],
	["sky_piercer", "관천쾌섬창", "연환자검 + 만금창", "적에게 꽂힌 뒤 연속 3회 찌르기, 마지막에 넓은 범위 폭발"],
	["flame_guard", "염화호신진", "호심검방 + 화염구 장판", "불타는 검기를 사방으로 훨씬 많이 흩뿌리고, 착탄 지점마다 이동속도를 늦추는 불바다를 남기는 화염 방어진"],
	["cataclysm_fury", "패왕귀멸진", "패왕붕권 + 귀곡저주", "정면으로 기의 주먹을 발사하는 광역 타격 + 가장 위협적인 적(보스 우선)을 저주로 낙인찍어 지속적으로 갉아먹는 복합 무기"],
]
...
const EVOLUTION_GUIDE := [
	...
	["패왕비급", "공격력 +12%", "패왕붕권→파천붕권 / 호심검방→금강불괴검방"],
