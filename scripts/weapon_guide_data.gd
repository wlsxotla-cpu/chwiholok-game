...
const BASE_WEAPON_GUIDE := [
	...
	["halberd", "패왕연편", "가장 먼 적 방향으로 채찍을 휘둘러 그 직선상의 모든 적을 타격하고 자신 쪽으로 끌어당김. 진악 전용 시작 무기."],
...
const FUSION_GUIDE := [
	...
	["cataclysm_fury", "패왕귀멸진", "패왕연편 + 귀곡저주", "주변을 강타하는 광역 후려치기 + 원거리 무작위 적에게 저주 낙뢰를 동시에 퍼붓는 합공진"],
]
...
const EVOLUTION_GUIDE := [
	...
	["패왕비급", "공격력 +12%", "패왕연편→파천연편 / 호심검방→금강불괴검방"],
